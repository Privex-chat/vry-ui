...
class Requests:
    ...
    def check_version(version, copy_run_update_script):
        ...
        try:
            r = requests.get("https://api.github.com/repos/Privex-chat/vry-ui/releases")
                ...
                for i in range(max_retries):
                    try:
                        ...
                        if response.status_code == 200 and response.json().get("errorCode") != "RPC_ERROR":
                            ...
                            self.log(f"Local API not ready (RPC_ERROR or status {response.status_code}). Retrying ({i+1}/{max_retries})...")
                            time.sleep(5)
